...
export class TeacherAddStudentComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        .map(value => {
            this.student.create(this.student.id=Math.random().toString(32).substring(2),
                this.teacher.id,0,value.firstName, value.lastName,value.level,
                value.username,value.password,this.student.dateRegistered = new Date().toLocaleString());
            return this.student;
        })
