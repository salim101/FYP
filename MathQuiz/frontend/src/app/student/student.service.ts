...
export class StudentService {
  ...
  getAdditionQuizzes = (student: Student) : Observable<Quiz[]> => {
    ...
    return this.http.get('/api/student/' + this.student['id'] + '/quizzes/addition', options).map(r => r.json());
  }

  getSubtractionQuizzes = (student: Student) : Observable<Quiz[]> => {
    let headers = new Headers({Authorization: 'Bearer ' + this.authenticationService.token});
    let options = new RequestOptions({headers: headers});
    ...
    params.set('teacherID', this.student.teacherID.toString());
    options.search = params;
    return this.http.get('/api/student/' + this.student['id'] + '/quizzes/subtraction', options).map(r => r.json());
  }

  getMultiplicationQuizzes = (student: Student) : Observable<Quiz[]> => {
    let headers = new Headers({Authorization: 'Bearer ' + this.authenticationService.token});
    let options = new RequestOptions({headers: headers});
    ...
    params.set('teacherID', this.student.teacherID.toString());
    options.search = params;
    return this.http.get('/api/student/' + this.student['id'] + '/quizzes/multiplication', options).map(r => r.json());
  }

  getDivisionQuizzes = (student: Student) : Observable<Quiz[]> => {
    let headers = new Headers({Authorization: 'Bearer ' + this.authenticationService.token});
    let options = new RequestOptions({headers: headers});
    ...
    params.set('teacherID', this.student.teacherID.toString());
    options.search = params;
    return this.http.get('/api/student/' + this.student['id'] + '/quizzes/division', options).map(r => r.json());
  }
